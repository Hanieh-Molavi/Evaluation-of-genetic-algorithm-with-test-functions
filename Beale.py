...
def beale(X):
    return (1.5-X[0] + X[0]*X[1])**2 + (2.25-X[0]+X[0]*X[1]**2)**2 + (2.2625-X[0]+X[0]*X[1]**3)**2
...
for i in range(0,20):        
    ...
    beale_model=ga(function=beale,dimension=2,variable_type='float',variable_boundaries=varbound)
                      
    
                            
    beale_model.run()
    sol.append(beale_model.output_dict)                      
...
count = epsilon_destince(destination,0.00000001)
